...
import { dialogsConfig } from '~/core/constants/dialogs'

export const useDialogManager = () => {
  const shownDialogs = useLocalStorage<
    Record<string, { version: number; lastShown: string }>
  >('shown-dialogs', {})
  ...
  const getNextDialog = (): DialogConfig | null => {
    const availableDialogs = dialogsConfig
      .filter(shouldShowDialog)
  ...
  const forceShowDialog = (dialogId: string) => {
    const dialog = dialogsConfig.find((d) => d.id === dialogId)
